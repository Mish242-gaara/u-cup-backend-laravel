...
@section('header', 'Modifier : ' . $player->full_name)
    ...
    <form action="{{ route('admin.players.update', $player) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $player->last_name) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('last_name') border-red-500 @enderror" required>
                @error('last_name')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                {{-- Input : bg-gray-700, border-gray-600, text-white, focus:ring-blue-400 --}}
                <input type="text" name="first_name" id="first_name" value="{{ old('first_name', $player->first_name) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('first_name') border-red-500 @enderror" required>
                @error('first_name')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                {{-- Input : bg-gray-700, border-gray-600, text-white, focus:ring-blue-400 --}}
                <input type="number" name="jersey_number" id="jersey_number" value="{{ old('jersey_number', $player->jersey_number) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('jersey_number') border-red-500 @enderror" placeholder="Ex: 10" min="1" max="99">
                @error('jersey_number')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="birth_date" id="birth_date" value="{{ old('birth_date', $player->birth_date ? $player->birth_date->format('Y-m-d') : '') }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('birth_date') border-red-500 @enderror">
                @error('birth_date')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                {{-- Input : bg-gray-700, border-gray-600, text-white, focus:ring-blue-400 --}}
                <input type="number" name="height" id="height" value="{{ old('height', $player->height) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('height') border-red-500 @enderror" placeholder="Ex: 180">
                @error('height')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                <label for="photo" class="block text-sm font-semibold text-white mb-1">Photo du Joueur (Remplacer)</label>
                
                @if($player->photo_path)
                    <div class="mb-4 flex items-center space-x-3">
                        <p class="text-sm text-gray-400">Photo actuelle :</p>
                        {{-- Image : border-gray-600 --}}
                        <img src="{{ asset('storage/' . $player->photo_path) }}" class="h-16 w-16 object-cover border-2 border-gray-600 rounded-lg p-1 shadow-sm">
                    </div>
                @endif
                ...
                >
                <p class="mt-1 text-xs text-gray-400">Laissez vide pour conserver l'image actuelle.</p>
                @error('photo')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-8 flex justify-end space-x-3">
            ...
            <button type="submit" class="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-500 transition font-semibold shadow-md">Mettre à jour le Joueur</button>
        </div>
    </form>
</div>
@endsection