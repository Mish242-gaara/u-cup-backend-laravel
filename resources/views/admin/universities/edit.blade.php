...
@section('header', 'Modifier : ' . $university->short_name)
    ...
    <form action="{{ route('admin.universities.update', $university) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $university->name) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('name') border-red-500 @enderror" required>
                @error('name')
            ...
            <div>
                <label class="block text-sm font-medium text-white mb-1">Abréviation (Sigle)</label>
                {{-- Input : bg-gray-700, border-gray-600, text-white, focus:ring-blue-400 --}}
                <input type="text" name="short_name" value="{{ old('short_name', $university->short_name) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('short_name') border-red-500 @enderror" required>
                @error('short_name')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @if($university->logo)
                    <div class="mb-2">
                        ...
                        <img src="{{ asset('storage/' . $university->logo) }}" class="h-16 w-16 object-contain border border-gray-600 rounded p-1">
                    </div>
                @endif
            ...
            <div>
                <label class="block text-sm font-medium text-white mb-1">Couleurs principales</label>
                {{-- Input : bg-gray-700, border-gray-600, text-white, focus:ring-blue-400 --}}
                <input type="text" name="colors" value="{{ old('colors', $university->colors) }}" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('colors') border-red-500 @enderror">
                @error('colors')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border-gray-600 rounded-lg shadow-sm focus:ring-blue-400 focus:border-blue-400 bg-gray-700 text-white @error('description') border-red-500 @enderror">{{ old('description', $university->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
